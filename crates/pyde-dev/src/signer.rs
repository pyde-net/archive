...
impl Signer {
    ...
    pub fn sign_transaction(&self, tx: &mut Transaction) -> Result<(), String> {
        let hash = tx.hash();
        let sig =
            falcon_sign(&self.secret_key, &hash).map_err(|e| format!("signing failed: {}", e))?;
    ...
    pub fn from_private_key(hex_str: &str) -> Result<Self, String> {
        ...
        let bytes = hex::decode(hex).map_err(|e| format!("invalid private key hex: {}", e))?;
...
pub fn resolve_signer(
    ...
    if private_key.is_some() && wallet_name.is_some() {
        return Err("cannot use both --private-key and --wallet. Choose one:\n\
...
fn load_devnet_account(index: usize) -> Result<Signer, String> {
    ...
    for path in &paths {
        if path.exists() {
            ...
            let account = keys.get(index).ok_or_else(|| {
                format!(
    ...
    Err("no devnet keys found. Start a devnet node first:\n\
         \n\
         $ pyde node --dev\n\
         \n\
