...
fn lex_erc20_contract() {
    ...
    let contracts = tokens.iter().filter(|t| t.kind == TokenKind::Contract).count();
    let storages = tokens.iter().filter(|t| t.kind == TokenKind::Storage).count();
    ...
    let returns = tokens.iter().filter(|t| t.kind == TokenKind::Return).count();
    let attrs: Vec<_> = tokens.iter().filter(|t| matches!(&t.kind, TokenKind::Attribute(_))).collect();
    ...
    // Check attributes
    let attr_names: Vec<&str> = attrs.iter().map(|t| {
        if let TokenKind::Attribute(s) = &t.kind { s.as_str() } else { "" }
    }).collect();
