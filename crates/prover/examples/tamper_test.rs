...
fn main() {
    ...
    macro_rules! tamper_test {
        ($name:expr, $code:expr, $tamper:expr) => {{
            ...
            match try_prove_verify(&mut tampered) {
                ...
                Err(e) => {
                    let short = if e.contains("prover rejected") {
                        "prover refused"
                    } else {
                        "verifier rejected"
                    };
    ...
    tamper_test!(
        "STORE: tamper stored value (42 → 99)",
        mem_code.clone(),
        |trace: &mut ExecutionTrace| {
    ...
    tamper_test!(
        "LOAD: tamper loaded value (42 → 99)",
        mem_code.clone(),
        |trace: &mut ExecutionTrace| {
    ...
    tamper_test!(
        "OPCODE BITS: tamper bit decomposition",
        add_code.clone(),
        |trace: &mut ExecutionTrace| {
    ...
    tamper_test!(
        "REGISTER SELECTOR: tamper rd_sel",
        add_code.clone(),
        |trace: &mut ExecutionTrace| {
    ...
    tamper_test!(
        "SHR: tamper gp[rd] without OP_RESULT",
        shr_code.clone(),
        |trace: &mut ExecutionTrace| {
    ...
    tamper_test!(
        "BNE: force branch_taken when equal",
        bne_code.clone(),
        |trace: &mut ExecutionTrace| {
    ...
    tamper_test!(
        "ADD: tamper rs1_sel (wrong source)",
        add_code.clone(),
        |trace: &mut ExecutionTrace| {
            // ADD r3, r1, r2 — change rs1_sel to point to r5 instead of r1
            for i in 0..16 {
                trace.rows[2].fields[col::rs1_sel(i)] = Goldilocks::zero();
            }
    ...
    tamper_test!(
        "AND: tamper gp[rd] only (mismatch with OP_RESULT)",
        and_code.clone(),
        |trace: &mut ExecutionTrace| {
    ...
    tamper_test!(
        "STORAGE: inject fake storage on non-storage op",
        add_code.clone(),
        |trace: &mut ExecutionTrace| {
    ...
    tamper_test!(
        "MEMORY INACTIVE: inject mem_val on non-memory op",
        add_code.clone(),
        |trace: &mut ExecutionTrace| {
    ...
    tamper_test!(
        "MEMORY INACTIVE: inject mem_addr on non-memory op",
        add_code.clone(),
        |trace: &mut ExecutionTrace| {
    ...
    tamper_test!(
        "BEQ: make operands unequal (break branch condition)",
        beq_code.clone(),
        |trace: &mut ExecutionTrace| {
    ...
    println!(
        "  Failed: {} (tampered traces incorrectly accepted!)",
        failed
    );
