...
fn full_production_benchmark() {
    ...
    println!(
        "  [SETUP] 4 validators: {:.0}ms",
        t0.elapsed().as_secs_f64() * 1000.0
    );

    let t0 = Instant::now();
    let mut accounts: Vec<Account> = (0..500)
        .into_par_iter()
        .map(|_| {
            let (pk, sk) = falcon_keygen().unwrap();
            let address = derive_eoa_address(pk.as_bytes());
        ...
        .collect();
    println!(
        "  [SETUP] 500 accounts (parallel keygen): {:.0}ms",
        t0.elapsed().as_secs_f64() * 1000.0
    );
    ...
    println!(
        "  [SETUP] Funded 500 accounts: {:.0}ms",
        t0.elapsed().as_secs_f64() * 1000.0
    );
    ...
    for spec in &specs {
        let bin = if spec.is_multi {
            compile_last(spec.source)
        } else {
            compile_single(spec.source)
        };
        compiled.insert(spec.name.to_string(), bin);
    }
    println!(
        "  [SETUP] Compiled {} contracts: {:.0}ms\n",
        specs.len(),
        t0.elapsed().as_secs_f64() * 1000.0
    );
    ...
    let exec_ms = t_exec.elapsed().as_secs_f64() * 1000.0;
    println!(
        "    Sign: {:.0}ms | Exec: {:.0}ms | {}/{} ok | {:.0} TPS",
        sign_ms,
        exec_ms,
        ok,
        deploy_txs.len(),
        ok as f64 / (exec_ms / 1000.0)
    );
    all_metrics.push(BlockMetrics {
        label: format!("Deploy {} contracts", specs.len()),
        tx_count: deploy_txs.len(),
        ok_count: ok,
        fail_count: fail,
        exec_ms,
        commit_ms: 0.0,
    ...
    println!(
        "    Sign: {:.0}ms | Exec: {:.1}ms | Commit: {:.0}ms | Total: {:.0}ms",
        sign_ms, exec_ms, commit_ms, total_ms
    );
    println!(
        "    {}/{} ok | {} groups | EXEC TPS: {:.0} | TOTAL TPS: {:.0}",
        ok,
        txs.len(),
        groups,
        3000.0 / (exec_ms / 1000.0),
        3000.0 / (total_ms / 1000.0)
    );
    all_metrics.push(BlockMetrics {
    ...
    sync_nonces(&mut accounts, &smt);
    println!(
        "\n  -- BLOCK {}: 200 vault deposits (payable + events + u256) --",
        slot
    );
    ...
    let exec_ms = t_exec.elapsed().as_secs_f64() * 1000.0;
    println!(
        "    Sign: {:.0}ms | Exec: {:.0}ms | {}/{} ok | Events: {} | {:.0} TPS",
        sign_ms,
        exec_ms,
        ok,
        txs.len(),
    ...
    // First add liquidity
    {
        ...
        let mut x = [0u8; 32];
        x[..8].copy_from_slice(&1_000_000u64.to_le_bytes());
        let mut y = [0u8; 32];
        y[..8].copy_from_slice(&2_000_000u64.to_le_bytes());
    ...
    sync_nonces(&mut accounts, &smt);
    println!(
        "\n  -- BLOCK {}: 200 multi-slot writes (8 sstores each) --",
        slot
    );
    ...
    println!(
        "  {:40} {:>6} {:>6} {:>8} {:>8} {:>10}",
        "Workload", "OK", "Fail", "Exec ms", "Sign ms", "TPS"
    );
    println!("  {}", "-".repeat(78));
    for m in &all_metrics {
        let tps = if m.exec_ms > 0.0 {
            m.ok_count as f64 / (m.exec_ms / 1000.0)
        } else {
            0.0
        };
        println!(
    ...
    println!(
        "  FALCON-512 sigs:      {}",
        all_metrics.iter().map(|m| m.tx_count).sum::<usize>() * 2
    );
    println!(
        "  Wall clock time:      {:.1}s",
