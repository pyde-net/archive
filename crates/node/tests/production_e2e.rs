...
fn production_signed_transfer() {
    ...
    let accounts_json: serde_json::Value = serde_json::from_str(
        &std::fs::read_to_string(&accounts_path).expect("read accounts.json")
    ...
    let accts = accounts_json["accounts"].as_array().expect("accounts array");
    // Use the first non-faucet account
    let sender_info = accts.iter()
    ...
    // Check balance
    let resp = rpc_call(&rpc, "pyde_getBalance", &format!("\"0x{}\"", hex::encode(address)));
    ...
    // Get nonce
    let resp = rpc_call(&rpc, "pyde_getTransactionCount", &format!("\"0x{}\"", hex::encode(address)));
    ...
    // Verify locally
    assert!(tx.verify_signature(pk.as_bytes()), "local signature verification failed");
    ...
    // Check receipt
    let resp = rpc_call(&rpc, "pyde_getTransactionReceipt", &format!("\"{}\"", submitted_hash));
    if let Some(result) = resp.get("result") {
        let success = result.get("success").and_then(|v| v.as_bool()).unwrap_or(false);
        println!("[{}] receipt: success={}", if success { "PASS" } else { "FAIL" }, success);
    ...
    let resp = rpc_call(&rpc, "pyde_getBalance", &format!("\"0x{}\"", hex::encode(recipient)));
    ...
    println!("[{}] recipient balance: {} (expected >= 1000000)", if new_bal >= 1_000_000 { "PASS" } else { "FAIL" }, new_bal);
...
fn production_signed_deploy_and_call() {
    let rpc = match std::env::var("PYDE_PRODUCTION_RPC") {
        Ok(url) => url,
        Err(_) => { println!("SKIP: PYDE_PRODUCTION_RPC not set"); return; }
    };
    let accounts_path = match std::env::var("PYDE_ACCOUNTS_JSON") {
        Ok(p) => p,
    ...
    let pk_hex = sender_info["privateKey"].as_str().unwrap().trim_start_matches("0x");
    ...
    let resp = rpc_call(&rpc, "pyde_getTransactionReceipt", &format!("\"{}\"", deploy_hash));
    ...
    let contract_hex = receipt.get("returnData").and_then(|v| v.as_str()).unwrap_or("");
    ...
    let resp = rpc_call(&rpc, "pyde_call", &format!(
        ...
        hex::encode(address), contract_hex, sel_hex
    ));
