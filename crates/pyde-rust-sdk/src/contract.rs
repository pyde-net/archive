...
impl ContractCall {
    ...
    /// ```
    pub fn arg_u8(self, val: u8) -> Self {
        self.arg_u64(val as u64)
    }
    ...
    /// ```
    pub fn arg_u16(self, val: u16) -> Self {
        self.arg_u64(val as u64)
    }
    ...
    /// ```
    pub fn arg_u32(self, val: u32) -> Self {
        self.arg_u64(val as u64)
    }
    ...
    /// ```
    pub fn arg_i8(self, val: i8) -> Self {
        self.arg_u64(val as i64 as u64)
    }
    ...
    /// ```
    pub fn arg_i16(self, val: i16) -> Self {
        self.arg_u64(val as i64 as u64)
    }
    ...
    /// ```
    pub fn arg_i32(self, val: i32) -> Self {
        self.arg_u64(val as i64 as u64)
    }
    ...
    /// ```
    pub fn arg_i64(self, val: i64) -> Self {
        self.arg_u64(val as u64)
    }
    ...
    pub fn arg_vec_u64(mut self, vals: &[u64]) -> Self {
        ...
        self.args
            .extend_from_slice(&(data_len as u64).to_le_bytes()); // byte_len
        self.args
            .extend_from_slice(&(vals.len() as u64).to_le_bytes()); // count
        self.args
            .extend_from_slice(&(vals.len() as u64).to_le_bytes()); // cap
        for v in vals {
    ...
    pub fn arg_vec_address(mut self, vals: &[[u8; 32]]) -> Self {
        ...
        self.args
            .extend_from_slice(&(data_len as u64).to_le_bytes());
        self.args
    ...
    pub fn arg_vec_of(
        mut self,
        count: usize,
        build_fn: impl FnOnce(ContractCall) -> ContractCall,
    ) -> Self {
    ...
    pub fn arg_struct(mut self, build_fn: impl FnOnce(ContractCall) -> ContractCall) -> Self {
        ...
        self.args
            .extend_from_slice(&(fields.len() as u64).to_le_bytes()); // byte_len
...
impl DeployData {
    ...
    pub fn from_json(json: &str, args: &serde_json::Value) -> std::result::Result<Self, String> {
        let v: serde_json::Value =
            serde_json::from_str(json).map_err(|e| format!("parse artifact: {}", e))?;
        let constructor_hex = v
        ...
        if let Some(fns) = abi.get("functions").and_then(|v| v.as_array()) {
            if let Some(ctor) = fns.iter().find(|f| {
                f.get("constructor")
